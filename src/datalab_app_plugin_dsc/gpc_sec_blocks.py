...
class GPCBlock(DataBlock):
    ...
    blocktype = "gpc"
    name = "GPC/SEC"
    description = "This block can plot GPC/SEC data from an excel file exported from Agilent GPC/SEC software "
    ...
    def parse_gpc_sheet(cls, filename: Path) -> pd.DataFrame:
        ...
        for i in range(len(gpcall)):
            if (
                isinstance(gpcall[gpcall.columns[0]][i], str)
                and "Channel Information" in gpcall[gpcall.columns[0]][i]
            ):
        ...
        gpccal = gpccal.rename(
            columns={"Administrative Information": "Column Calibration", "Unnamed: 1": "Info"}
        )
        ...
        gpcflow = gpcflow.rename(
            columns={"Administrative Information": "Flowrate Information", "Unnamed: 1": "Info"}
        )
    ...
    def _format_gpc_plot(
        ...
        bonusyselect = Select(
            title="Second y axis?", value="None", options=bonusymenu, height_policy="min"
        )
        # The bonus axis
        bonusy = []
        bonusplot = []
        ...
        bonusyselect.js_on_change(
            "value",
            CustomJS(
                args=dict(bonusy=bonusy, bonusymenu=bonusymenu, bonusplot=bonusplot),
                code="""
        ...
        prodcheckbox = CheckboxGroup(
            labels=[
                "Show slice regions?",
                "Show baseline regions?",
                "Show flow rate marker information?",
            ],
            active=[0, 1],
            height_policy="min",
        )
        ...
        for i in range(npeaks):
            ...
            shades.append(
                BoxAnnotation(
                    left=slicebounds[i][0],
                    right=slicebounds[i][1],
                    fill_color=shadecol,
                    fill_alpha=0.1,
                    hatch_pattern=hatchpat,
                    hatch_alpha=0.1,
                    level="underlay",
                    line_color="grey",
                    line_alpha=0.1,
        ...
        calcheckbox = CheckboxGroup(
            labels=["Show calibration curve?"], active=[0], sizing_mode="stretch_width"
        )
        ...
        if calinfo["Type"] == "Polynomial":
            ...
            callayout.x_range = Range1d(
                min(gpc_data["Retention Time"]), max(gpc_data["Retention Time"])
            )
        ...
        calcheckbox.js_on_click(
            CustomJS(
                args=dict(callayout=callayout, calcheckbox=calcheckbox),
                code="""
    ...
    def generate_gpc_plot(self):
        ...
        if "file_id" not in self.data:
            ...
            gpc_data, gpcmw, slicebounds, npeaks, calinfo, fminfo, gpcbase = self.parse_gpc_sheet(
                Path(file_info["location"])
            )
        ...
        if gpc_data is not None:
            fulllayout = self._format_gpc_plot(
                gpc_data, gpcmw, slicebounds, npeaks, calinfo, fminfo, gpcbase
            )
            self.data["bokeh_plot_data"] = bokeh.embed.json_item(
                fulllayout, theme=DATALAB_BOKEH_THEME
            )
